...
prompt_prep_1 = """Voici ci-dessous un systeme de notation en 8 sections pour evaluer une idee de projet, j'aimerais que tu l'utilises pour evaluer les idees de projets que je vais te donner.
...
prompt_prep_2 = """Quand je te donne une description de projet, peux-tu appliquer le systeme de notation que je t'ai decrit pour affecter une note a chaque partie ?
...
# Ex 2eme input : Le projet va couter 1M€ et permet de générer 500k€ d'économie chaque année grâce à l'augmentation de productivité des agents planificateurs. De plus, l'outil sera utilisé chaque mois, pour faire les plannings des 1000 hotesses et stewards de la compagnie Air France. L'équipe projet menant les développements et assurant le déploiement auprès des agents planificateurs est composée de 4 ingénieurs data scientists, rattachés aux équipes RO d'Air France.
...
def get_bot_response(user_input):
    ...
    total_score = 0
    if match:
        total_score = int(match.group(1))  # Extract the number after the tag
...
def summarizeProject():
    ...
    projectSummary_l = ""
    if beginning_of_chat:
        projectSummary_l = "Pas assez d'information partagee pour le moment"
        return projectSummary_l
    
    # Prep AI engine to print the document
    conversation_history.append({
        "role": "user",
